        ...
        match t {
            ...
            Action::PressKey(key) => {
                ...
                match &starting_state.expected_pin_hash {
                    ...
                    Auth::Authenticated => {
                        if *key == Key::Enter {
                            let temp: u64 = starting_state.keystroke_register.iter().map(|k| <Key as Into<&str>>::into(*k)).collect::<String>().parse().unwrap_or(0);
                            if temp <= starting_state.cash_inside {
                                Self {
                                    cash_inside: starting_state.cash_inside - temp,
                                    expected_pin_hash: Auth::Waiting,
                                    keystroke_register: Vec::new(),
                                }
                            } else {
